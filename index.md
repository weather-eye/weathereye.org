...
> ⚠️ **Caution:** Not Implemented
> - Install on a dedicated server that has no operating system installed (requires physical access to machine and USB memory stick): [[WeatherEye OS]]
> - Install on an existing Linux server, including in the cloud (requires SSH access to remote machine): [[WeatherEye Controller]]
> - Install [[WeatherEye Setup]] on your local MacOS, Windows or Linux desktop machine in order to provision servers and then install and configure WeatherEye
